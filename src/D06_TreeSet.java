...
public class D06_TreeSet {
	 ...
	 * # TreeSet
	 * 
	 * - 중복값을 저장할 수 없다 - 데이터를 저장할 때 Comparator를 이용해 미리 정렬한다 - 요소로 null을 허용하지 않는다 -
	 * 처음에 정렬해서 넣기 때문에 HashSet에 비해 저장 속도가 느리다 - 하지만 TreeSet은 미리 되어 있기 때문에 정렬된 순서로
	 * 데이터를 꺼내는것은 가장 빠르다
	 */
	...
	public static void main(String[] args) {
		...
		// decendingSet() : 미리 정렬해놓은 방향의 반대방향인 Set 인스턴스를 반환
		...
		// subSet(formElement, toElement):원하는 요소부터 시작해서 원하는 요소까지 자르는
		// Set을 반환
		System.out.println(subSet(new Grape().setGno("3"), new Grape().setGno(1)));
...
class MangoCompareBySweetAsc implements Comparator<Mango> {
	 ...
	 * # Comparator의 규칙
	 * 
	 * - compare 메서드의 리턴 값이 1이면 o1과 o2의 순서를 바꾼다 - compare 메서드의 리턴 값이 0이면 같은 크기로 취급한다
